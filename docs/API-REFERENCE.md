...
**フィールド説明:**
- `projects`: プロジェクト作成のクォータ
- `messages`: メッセージ送信のクォータ
- `plan`: ユーザーのプラン (`free`, `pro`, `team`)
- `resetAt`: クォータリセット時刻（UTC 00:00 = JST 09:00）
- `limit: -1` は無制限を示す（`pro`, `team` プラン）

**プラン別制限:**

| プラン | プロジェクト作成 | メッセージ送信 |
|--------|------------------|----------------|
| free | 3回/日 | 20回/日 |
| pro | 100回/日 | 500回/日 |
| team | 無制限 | 無制限 |
